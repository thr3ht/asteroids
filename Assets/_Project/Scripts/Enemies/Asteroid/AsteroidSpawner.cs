...
namespace _Project.Scripts.Enemies.Asteroid
{
    public class AsteroidSpawner
    {
        ...
        public AsteroidSpawner(AsteroidFactory factory, WorldBounds bounds, EnemyConfig config)
        {
            _factory = factory;
            _bounds = bounds;
            _config = config;
            _timeUntilNextSpawn = _config.AsteroidSpawnInterval;
        }
        ...
        {
            for (int i = 0; i < _config.AsteroidInitialCount; i++)
            {
                SpawnAsteroid();
            }
        }
